...
import type { FoodType, UserProfile, ZodiacElement } from "@/types/domain";
...
export async function GET(request: NextRequest) {
  ...
  if (foodType && VALID_FOOD_TYPES.has(foodType)) {
    items = items.filter((user) => user.foodPreferences.includes(foodType as FoodType));
  }

  if (area) {
    const normalizedArea = normalizeAreaName(area);
    items = items.filter(
      (user) => normalizeAreaName(user.preferredArea) === normalizedArea
    );
  }

  if (zodiacElement && VALID_ELEMENTS.has(zodiacElement)) {
    items = items.filter(
      (user) => user.zodiacElement === (zodiacElement as ZodiacElement)
    );
  }
